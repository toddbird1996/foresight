  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-4 py-4">
        ...
        {expenses.length > 0 && (
          <button onClick={() => {
            const lines = expenses.map(e => `${e.expense_date}\t${e.title}\t$${parseFloat(e.amount).toFixed(2)}\t${e.paid_by === 'me' ? 'I paid' : 'Other parent paid'}\t${e.category}\tMy share: $${(parseFloat(e.amount) * (e.my_share_percent || 50) / 100).toFixed(2)}`);
            const header = `SHARED EXPENSE REPORT\nExported: ${new Date().toLocaleDateString('en-CA')}\nTotal: $${totalSpent.toFixed(2)} | I Paid: $${iPaid.toFixed(2)} | They Paid: $${theyPaid.toFixed(2)}\nBalance: ${balance >= 0 ? 'They owe me' : 'I owe them'} $${Math.abs(balance).toFixed(2)}\n${'─'.repeat(50)}\nDate\tDescription\tAmount\tPaid By\tCategory\tMy Share\n`;
            const blob = new Blob([header + lines.join('\n')], { type: 'text/plain' });
            const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
            a.download = `expenses-${new Date().toISOString().split('T')[0]}.txt`; a.click();
